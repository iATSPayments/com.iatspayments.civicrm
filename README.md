...
Caution on use of Pricesets in recurring contributions - this extension transacts the total amount but does not distribute the amount over the various line items. This is not easy to do - due to a bug in CiviCRM Core API (to create lineitems) - and secondly - it would be difficicult to do when the total amount of a recurring contribution has changed - that field can be changed in CiviCRM independently of any line items - it will be impossible to know how much would have to be allocated to which line item. This is a bookkeeping issue - not a transaction issue. Though not always ideal - an easy workaround would be to do different transactions for different type of CiviCRM payments instead. 
