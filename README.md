...
Caution on use of Pricesets in recurring contributions - this extension transacts the total amount but does not distribute the amount over the various line items. For several reasons this is not easy to do: a) a bug in CiviCRM Core API (prevents successful creation of line items) - and b) - the total amount of a recurring contribution can change - that field can be changed in CiviCRM independently of any line items - it will be impossible to know how much would have to be allocated to which line item. Is the additional $20 a Donation or is it to cover an increase in Membership fees? This is a bookkeeping issue - not a transaction issue. Though not always ideal - an easy workaround (and one both your bookkeeper and accountant will be very happy with) is to do different transactions for different type of CiviCRM payments instead. 
